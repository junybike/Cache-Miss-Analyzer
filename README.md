...
source.cpp ──► ast_analyzer ────┘   (correlate)    (pattern match)          (improves code)
               (AST→JSON)
```
...
`llm_integration.py` takes the original code and JSON file generated by `recommend.py` to give code improvement instruction to an LLM. It can edits the original code, makes a copy of the improved code, or print the improvement instruction.   
...
# --llm option: [claude, chatgpt, gemini], --mode option: [edit, copy, instruction], --api-key: replace k with api key
./profiler/run_perf.sh examples/comprehensive.cpp examples/comprehensive --llm claude --mode edit --api-key key
```
...
# llm_integration --llm option: [claude, chatgpt, gemini], --mode option: [edit, copy, instruction], --api-key: replace k with api key
python3 analysis/llm_integration.py data/results/recommendations.json examples/comprehensive.cpp --llm claude --mode edit --api-key k
```
...
│   └── llm_integration.py    passes code improvement instruction to LLM and edits code
├── examples/
