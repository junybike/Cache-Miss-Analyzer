...
struct Waypoint
{
    ...
    float y;
    float cost;
    int next; // index of next waypoint, -1 if end
};

std::vector<Waypoint> build_path(int n)
{
...
int main()
{
    ...
    std::vector<Waypoint> path = build_path(N_WAYPOINTS);

    float total_cost = 0.0f;
    int curr = N_WAYPOINTS - 1;
    while (curr != -1)
    {
        total_cost += path[curr].cost;
        curr = path[curr].next;
    }
    ...
    std::vector<float> projectile_x(N_PROJECTILES);
    std::vector<float> projectile_y(N_PROJECTILES);

    for (int i = 0; i < N_PROJECTILES; i++)
    {
        projectile_x[i] = static_cast<float>(i) * 0.1f;
        projectile_y[i] = static_cast<float>(i) * 0.2f;
    }
    ...
    for (int pass = 0; pass < 10; pass++)
    {
        for (int i = 0; i < N_PROJECTILES; i++)
        {
            float dx = projectile_x[i];
            float dy = projectile_y[i];
    ...
    std::vector<float> entity_x(N_ENTITIES);
    std::vector<float> entity_vx(N_ENTITIES);

    for (int i = 0; i < N_ENTITIES; i++)
    {
        entity_x[i]  = static_cast<float>(i);
        entity_vx[i] = 1.0f;
    }
    ...
    std::cout << "entity[0] pos: " << entity_x[0] << std::endl;
    ...
    std::vector<int> sorted_ranking = ranking;
    std::sort(sorted_ranking.begin(), sorted_ranking.end());

    std::vector<int> temp_scores(N_LOOKUPS);
    for (int i = 0; i < N_LOOKUPS; i++)
        temp_scores[i] = scores[sorted_ranking[i]];

    long score_total = 0;
    for (int i = 0; i < N_LOOKUPS; i++)
        score_total += temp_scores[i];
